...
def main(spark, model_size, ranks):
    ...
    for rank in ranks:
        ...
        print(f"Running Model: Max Iter:{10}, Rank: {rank}, Reg Param: {0.1}")
        # Pass through dictionary of keyword arguments to Model()
        reccomender_system = Model(model_size=model_size, model_type='als', rank=int(rank), maxIter = 10,regParam = 0.1)
        # Run the model
